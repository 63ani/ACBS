...
class GameState():
	...
	def makeMove(self, move):
		...
		if move.enPassant:
			...
			# put the write for enPassant to COM
		...
		if move.isCastleMove:
			if move.endCol == 6:  # King-side castling
				if move.pieceMoved == 'wK':
					self.board[7][5] = 'wR'  # Move white rook
					self.board[7][7] = '--'  # Empty original rook position
					print('O-O-O')
				else:
					self.board[0][5] = 'bR'  # Move black rook
					self.board[0][7] = '--'  # Empty original rook position
					print('O-O-O')
			elif move.endCol == 2:  # Queen-side castling
				if move.pieceMoved == 'wK':
					self.board[7][3] = 'wR'  # Move white rook
					self.board[7][0] = '--'  # Empty original rook position
					print('O-O')
				else:
					self.board[0][3] = 'bR'  # Move black rook
					self.board[0][0] = '--'  # Empty original rook position
					print('O-O')

		# Update the board for the moved piece
   ...
   '''
	...
	def getNumPieces(self):
		...
		for row in self.board:
			num_pieces += row.count('--')  # Count empty squares
		return 32 - num_pieces  # Total number of pieces is 32
